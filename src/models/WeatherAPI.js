...
export default class {
  ...
  async getCurrent(scale, language, parametr) {
    ...
    const weather = new Weather(dayOfWeek, parseFloat(data.temp), data.weather.icon, data.weather.description,
      parseFloat(data.app_temp), parseFloat(data.wind_spd), parseFloat(data.rh));
    const country = this.getCountryName(data.country_code);
    const cityInfo = new CityInfo(parseFloat(data.lat), parseFloat(data.lon),
      data.timezone, data.city_name, country, weather);
    return cityInfo;
  }
  ...
  async getForecast(scale, language, parametr) {
    ...
    for (let i = 1; i < 4; i += 1) {
      ...
      const dayOfWeek = new Date(data.datetime).getDay();
      const weather = new Weather(dayOfWeek, parseFloat(data.temp), data.weather.icon);
  ...
  async getWeather(scale, language, parametr) {
    try {
      ...
      return new Response(true, values[0]);
    } catch (e) {
      return new Response(false, `Ошибка получения информации о погоде: ${e}`);
    }
  }
}
